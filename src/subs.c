...
static int subs__send(struct mosquitto__subleaf *leaf, const char *topic,
                      uint8_t qos, int retain,
                      struct mosquitto_msg_store *stored, uint16_t messageID) {
  bool client_retain;
  uint16_t mid;
  ...
  rc2 = mosquitto_acl_check(leaf->context, topic, stored->payloadlen,
                            stored->payload, stored->qos, stored->retain,
                            MOSQ_ACL_READ);
  if (rc2 == MOSQ_ERR_ACL_DENIED) {
    ...
    if (leaf->identifier) {
      mosquitto_property_add_varint(
          &properties, MQTT_PROP_SUBSCRIPTION_IDENTIFIER, leaf->identifier);
    }
    mid = messageID;
    if (db__message_insert(leaf->context, mid, mosq_md_out, msg_qos,
                           client_retain, stored, properties, true) == 1) {
      return 1;
    }
  } else {
...
static int subs__shared_process(struct mosquitto__subhier *hier,
                                const char *topic, uint8_t qos, int retain,
...
static int subs__process(struct mosquitto__subhier *hier, const char *source_id,
                         const char *topic, uint8_t qos, int retain,
                         struct mosquitto_msg_store *stored,
                         uint16_t messageID) {
  int rc = 0;
  ...
  while (source_id && leaf) {
    if (!leaf->context->id ||
        (leaf->no_local && !strcmp(leaf->context->id, source_id))) {
      leaf = leaf->next;
      continue;
    }
    rc2 = subs__send(leaf, topic, qos, retain, stored, messageID);
...
static int sub__add_leaf(struct mosquitto *context, uint8_t qos,
                         uint32_t identifier, int options,
                         struct mosquitto__subleaf **head,
  ...
  *newleaf = NULL;
  ...
  while (leaf) {
    if (leaf->context && leaf->context->id &&
        !strcmp(leaf->context->id, context->id)) {
  ...
  leaf->retain_as_published =
      ((options & MQTT_SUB_OPT_RETAIN_AS_PUBLISHED) != 0);
...
static void sub__remove_shared_leaf(struct mosquitto__subhier *subhier,
...
static int sub__add_shared(struct mosquitto *context, const char *sub,
                           uint8_t qos, uint32_t identifier, int options,
  ...
  rc =
      sub__add_leaf(context, qos, identifier, options, &shared->subs, &newleaf);
  if (rc > 0) {
    if (shared->subs == NULL) {
      HASH_DELETE(hh, subhier->shared, shared);
      mosquitto__free(shared->name);
  ...
  if (rc != MOSQ_ERR_SUB_EXISTS) {
    slen = strlen(sub);
    csub =
        mosquitto__calloc(1, sizeof(struct mosquitto__client_sub) + slen + 1);
    ...
    if (i == context->sub_count) {
      subs = mosquitto__realloc(context->subs,
                                sizeof(struct mosquitto__client_sub *) *
                                    (size_t)(context->sub_count + 1));
      if (!subs) {
...
static int sub__add_normal(struct mosquitto *context, const char *sub,
                           uint8_t qos, uint32_t identifier, int options,
                           struct mosquitto__subhier *subhier) {
  ...
  rc = sub__add_leaf(context, qos, identifier, options, &subhier->subs,
                     &newleaf);
  if (rc > 0) {
    return rc;
  }
...
static int sub__add_context(struct mosquitto *context, const char *topic_filter,
                            uint8_t qos, uint32_t identifier, int options,
                            struct mosquitto__subhier *subhier,
                            char *const *const topics, const char *sharename) {
  ...
  while (topics && topics[topic_index] != NULL) {
    ...
    if (!branch) {
      /* Not found */
      branch = sub__add_hier_entry(subhier, &subhier->children,
                                   topics[topic_index], (uint16_t)topiclen);
  ...
  if (context && context->id) {
    if (sharename) {
      return sub__add_shared(context, topic_filter, qos, identifier, options,
                             subhier, sharename);
    } else {
      return sub__add_normal(context, topic_filter, qos, identifier, options,
                             subhier);
    }
  } else {
    return MOSQ_ERR_SUCCESS;
...
static int sub__remove_normal(struct mosquitto *context,
                              struct mosquitto__subhier *subhier,
                              uint8_t *reason) {
  struct mosquitto__subleaf *leaf;
  int i;
...
static int sub__remove_shared(struct mosquitto *context,
                              struct mosquitto__subhier *subhier,
                              uint8_t *reason, const char *sharename) {
  struct mosquitto__subshared *shared;
  struct mosquitto__subleaf *leaf;
  int i;
...
static int sub__remove_recurse(struct mosquitto *context,
                               struct mosquitto__subhier *subhier,
                               char **topics, uint8_t *reason,
                               const char *sharename) {
  struct mosquitto__subhier *branch;
...
static int sub__search(struct mosquitto__subhier *subhier, char **split_topics,
                       const char *source_id, const char *topic, uint8_t qos,
  ...
  if (split_topics && split_topics[0]) {
    ...
    HASH_FIND(hh, subhier->children, split_topics[0], strlen(split_topics[0]),
              branch);

    if (branch) {
      rc = sub__search(branch, &(split_topics[1]), source_id, topic, qos,
                       retain, stored, messageID);
  ...
  if (branch && !branch->children) {
     ...
     */
    rc =
        subs__process(branch, source_id, topic, qos, retain, stored, messageID);
    if (rc == MOSQ_ERR_SUCCESS) {
      have_subscribers = true;
    } else if (rc != MOSQ_ERR_NO_SUBSCRIBERS) {
      return rc;
...
struct mosquitto__subhier *sub__add_hier_entry(
    struct mosquitto__subhier *parent, struct mosquitto__subhier **sibling,
...
int sub__add(struct mosquitto *context, const char *sub, uint8_t qos,
             uint32_t identifier, int options) {
  int rc = 0;
  struct mosquitto__subhier *subhier;
  ...
  if (sharename) {
    ...
    if (!subhier) {
      subhier = sub__add_hier_entry(NULL, &db.shared_subs, topics[0],
                                    (uint16_t)topiclen);
      if (!subhier) {
        mosquitto__free(local_sub);
        mosquitto__free(topics);
    ...
    if (!subhier) {
      subhier = sub__add_hier_entry(NULL, &db.normal_subs, topics[0],
                                    (uint16_t)topiclen);
      if (!subhier) {
        mosquitto__free(local_sub);
        mosquitto__free(topics);
  ...
  rc = sub__add_context(context, sub, qos, identifier, options, subhier, topics,
                        sharename);
...
int sub__messages_queue(const char *source_id, const char *topic, uint8_t qos,
  ...
  */
  ...
  if (subhier) {
    ...
    messageID++;
    rc_normal = sub__search(subhier, split_topics, source_id, topic, qos,
                            retain, *stored, messageID);
    if (rc_normal > 0) {
      rc = rc_normal;
      goto end;
  ...
  if (subhier) {
    static uint16_t messageID = 0;
    messageID++;
    rc_shared = sub__search(subhier, split_topics, source_id, topic, qos,
                            retain, *stored, messageID);
    if (rc_shared > 0) {
      rc = rc_shared;
      goto end;
  ...
  if (rc_normal == MOSQ_ERR_NO_SUBSCRIBERS &&
      rc_shared == MOSQ_ERR_NO_SUBSCRIBERS) {
...
/* Remove a subhier element, and return its parent if that needs freeing as
 * well. */
static struct mosquitto__subhier *tmp_remove_subs(
  ...
  if (parent->subs == NULL && parent->children == NULL &&
      parent->shared == NULL && parent->parent) {
    return parent;
  } else {
    return NULL;
  }
...
int sub__clean_session(struct mosquitto *context) {
  ...
  for (i = 0; i < context->sub_count; i++) {
    ...
    if (context->subs[i]->shared) {
      ...
      while (leaf) {
        if (leaf->context == context) {
          ...
          db.shared_subscription_count--;
#endif
          sub__remove_shared_leaf(context->subs[i]->hier,
                                  context->subs[i]->shared, leaf);
          break;
        }
        leaf = leaf->next;
      }
    ...
    if (hier->subs == NULL && hier->children == NULL && hier->shared == NULL &&
        hier->parent) {
      do {
